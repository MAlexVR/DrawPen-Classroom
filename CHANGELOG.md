...
- Add and package an X11 input-shape helper for Fedora 44 COSMIC so Pointer Mode passes clicks through the transparent drawing surface while keeping the toolbar interactive.
- Preserve the toolbar anchor across Pointer/Draw mode changes and application restarts.
- Keep compact and expanded toolbars aligned and prevent first-toggle centering or vertical drift under COSMIC.
- Reduce black overlay and trailing artifacts while dragging the toolbar.

### Classroom tools

- Add larger configurable boards with white, cream, blue, slate, and black themes.
- Add plain, dots, lines, grid, and polka board patterns with adjustable spacing and opacity.
- Add full-screen capture to `Pictures/Screenshots`.
- Add a continuous 2–32 px pen-width slider with cursor preview.
- Add a labelled custom color control shared by drawing tools.
- Add line, square, rectangle, circle, oval, triangle, configurable table/matrix, and numbered-line tools.
- Add configurable numbered-line bounds, automatic horizontal/vertical layout, larger labels, and extra label-to-axis spacing.
- Add bold text editing through the toolbar and `Ctrl+B`.

### Distribution

- Provide an x86_64 RPM built and tested on Fedora 44 COSMIC using Electron's X11 compatibility backend.
- Document the derivative's upstream relationship, compatibility scope, artifact checksum, and MIT attribution.
