...
const getTrianglePoints = (figure) => {
...
const isOnTriangle = (x, y, figure) => {
  const tolerance = 5 + widthList[figure.widthIndex].figure_size / 2
  const trianglePoints = getTrianglePoints(figure)

  return isOnCurve(x, y, [...trianglePoints, trianglePoints[0]], tolerance)
}

const isOnTable = (x, y, figure) => {
  if (isOnRectangle(x, y, figure)) return true

  const { points, widthIndex, rows = 3, columns = 3 } = figure
  const [startX, startY] = points[0]
  const [endX, endY] = points[1]
  const minX = Math.min(startX, endX)
  const maxX = Math.max(startX, endX)
  const minY = Math.min(startY, endY)
  const maxY = Math.max(startY, endY)
  const tolerance = 5 + widthList[widthIndex].figure_size / 2

  if (y >= minY && y <= maxY) {
    for (let column = 1; column < columns; column += 1) {
      const lineX = minX + ((maxX - minX) * column / columns)
      if (Math.abs(x - lineX) <= tolerance) return true
    }
  }

  if (x >= minX && x <= maxX) {
    for (let row = 1; row < rows; row += 1) {
      const lineY = minY + ((maxY - minY) * row / rows)
      if (Math.abs(y - lineY) <= tolerance) return true
    }
  }
...
export const isOverFigure = (x, y, figure) => {
  switch (figure.type) {
    ...
    case 'triangle':
      return isOnPolygon(x, y, getTrianglePoints(figure))
    // case 'text':
    //   return isOverText(x, y, figure)
    default:
      return false
...
const isSegmentTouchTriangle = (segmentPoints, figure) => {
  const [eraseAtX, eraseAtY] = segmentPoints.at(-1)

  if (isOnTriangle(eraseAtX, eraseAtY, figure)) {
    return true
  }

  const trianglePoints = getTrianglePoints(figure)
  return isSegmentIntersectCurve(segmentPoints, [...trianglePoints, trianglePoints[0]])
}
...
export const areFiguresIntersecting = (eraserFigure, figure) => {
  switch (figure.type) {
    ...
    case 'triangle':
      return isSegmentTouchTriangle(eraserFigure.points, figure)
    case 'table':
      return isOnTable(...eraserFigure.points.at(-1), figure) || isSegmentTouchRectangle(eraserFigure.points, figure)
    case 'oval':
    case 'circle':
...
export const resizeFigure = (figure, activeFigureInfo, { x, y, isShiftPressed }) => {
  ...
  if (figure.type === 'number_line') {
    const stationaryPoint = resizingDotName === 'pointA' ? figure.points[1] : figure.points[0]
    const deltaX = x - stationaryPoint[0]
    const deltaY = y - stationaryPoint[1]
  ...
  if (['square', 'circle'].includes(figure.type)) {
    const pointA = figure.points[0]
    const pointB = figure.points[1]
    const pointC = [figure.points[0][0], figure.points[1][1]]
    const pointD = [figure.points[1][0], figure.points[0][1]]
    let startPoint
